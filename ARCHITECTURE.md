...
## 13. قرارات معمارية مطلوبة قبل البرمجة المكثفة

1. `ADR-001`: لغة الخادم وإطار API.
2. `ADR-002`: تقنية الواجهة وبروتوكول بث الأحداث.
3. `ADR-003`: مزود النموذج وواجهة التبديل بين المزودين.
4. `ADR-004`: طابور الأعمال ونموذج الاستئناف.
5. `ADR-005`: تخزين الأسرار والاتصالات.
6. `ADR-006`: أول أداة خارجية وحدود صلاحياتها.
7. `ADR-007`: سياسة الذاكرة والاحتفاظ والحذف.
